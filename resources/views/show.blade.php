...
<body>
    ...
    <div
        ...
        <textarea id="licenseCode" cols="80" rows="5" readonly
            style="width: 100%; padding: 12px; border-radius: 8px; border: 1px solid #cbd5e1; background: #f9fafb; font-size: 1rem; margin-bottom: 12px; resize: none;">{{ $licenca->license_code }}</textarea>
        <p style="color: #4b5563; margin-bottom: 20px;">Envie este código ao cliente para ativar o software.</p>
        <div style="display: flex; gap: 12px;">
            <button onclick="copiarCodigo()"
                style="flex: 1; background: #2563eb; color: #fff; border: none; padding: 10px 0; border-radius: 6px; font-size: 1rem; cursor: pointer; transition: background 0.2s;">Copiar</button>
            <a href="{{ route('server.create') }}"
                style="flex: 1; display: inline-block; text-align: center; background: #f3f4f6; color: #2563eb; border: 1px solid #2563eb; padding: 10px 0; border-radius: 6px; font-size: 1rem; text-decoration: none; transition: background 0.2s;">Gerar
                nova licença</a>
        </div>
    </div>
</body>
